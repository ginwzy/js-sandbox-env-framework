...
/**
 * 命令入口:
 *   mimic run     <script> [--profile name] [--url 目标域] [--trace] [--debug]
...
async function cmdRun([script], flags) {
	if (!script)
		return fail(
			"用法: sdenv run <script> [--profile name] [--url 目标域] [--trace] [--debug]",
		);
...
async function cmdDiff([profile], flags) {
	const { runDiff, formatReport, listBaselines } = await import(
		"../harness/index.js"
	);
	try {
...
async function cmdBaseline(_rest, flags) {
	console.error(
		"[弃用] `mimic baseline` 已并入 `mimic capture`(统一服务一次访问同源产 profile + 结构基线)。转启 capture。",
	);
	const { startCapture } = await import("../capture/server.js");
	startCapture({ port: Number(flags.port) || 8970 });
}
...
Promise.resolve()
	.then(() =>
		(
			...
			(() =>
				fail(
					"命令: run | check | capture | diff | baseline | serve | profiles",
				))
		)(rest, flags),
	)
