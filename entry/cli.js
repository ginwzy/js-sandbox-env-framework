...
async function cmdRun([script], flags) {
  if (!script) return fail('用法: sdenv run <script> [--profile name] [--url 目标域] [--trace] [--debug]');
  const code = fs.readFileSync(script, 'utf-8');
  const url = typeof flags.url === 'string' ? flags.url : undefined;
  const realm = await Realm.create({ profile: flags.profile, trace: !!flags.trace, url, debug: !!flags.debug });
