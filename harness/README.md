...
```bash
# 1) 先采真机全量基线(目标设备 Chrome 访问后落盘 harness/baselines/<name>.json)
npm run baseline                              # 起服务,目标真机 Chrome 访问采集

# 2) 用真机基线比对(--baseline 省略则取 baselines/ 下第一个;基线平台应与伪装 profile 对齐)
node entry/cli.js diff linux-chrome --baseline linux-chrome-v143
node entry/cli.js diff linux-chrome --baseline linux-chrome-v143 --t1       # 只就 T1 已修目标判 gate(.13 验收口径)
node entry/cli.js diff linux-chrome --baseline linux-chrome-v143 --verbose  # 含 INFO + 逐条 MISSING
node entry/cli.js diff linux-chrome --baseline linux-chrome-v143 --json     # 机器可读(摘要 + entries),供 CI
```
...
baselines/      真机基线 JSON(经统一采集服务 `mimic capture` 由真机 Chrome 采集,complete:true 全量)
mimic-snapshot.js  在 mimic realm 内跑 probe 取快照
index.js        runDiff 编排 + 报告格式化(profile 省略时按同名自动配对 baseline↔profile)
server.js       结构基线落盘 + host/命名派生(库模块;采集服务已并入 capture/,见 capture/server.js)
```

## 边界

- diff 只能比"你想到去探的属性",想不到的仍会漏。
- 基线只覆盖 probe 探到的 target;基线平台须与伪装 profile 对齐,否则平台相关属性(touch/deviceMemory/connection 等)会产生无法判定的 EXTRA 噪声。
