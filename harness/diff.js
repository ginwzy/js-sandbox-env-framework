 ...
 *   MISSING baseline 有、mimic 无 —— jsdom 天生缺几百个 API,预期内的覆盖缺口,永不致 gate 失败。
 ...
 * 避免一墙红噪声淹没真正该修的 tell —— 正是本 harness 的立项动机。
 *
...
function diffFn(target, key, prefix, base, mim, out) {
  ...
  if (!mim) {
    ...
    // 整函数 target 缺失才是 jsdom 覆盖缺口 → MISSING(由覆盖缺口白名单规则吞)。
 ...
 * @param {Array} entries
 * @param {object} [opts]
 * @param {boolean} [opts.t1Only]  仅就 T1 已修目标判 gate(harness 验收口径)
 */
